jQuery(document).ready(function() {
    ...
    if ( (typeof openlab_nr_actions_required !== 'undefined') && (openlab_nr_actions_required != '0') ) {
        jQuery('li.openlab-txtd-w-red-tab a').append('<span class="openlab-txtd-actions-count">' + openlab_nr_actions_required + '</span>');
    }
    ...
    jQuery(".openlab-dismiss-required-action").click(function(){
        ...
        jQuery.ajax({
            ...
            beforeSend : function(data,settings){
				jQuery('.openlab-txtd-tab-pane#actions_required h1').append('<div id="temp_load" style="text-align:center"><img src="' + openlabLiteWelcomeScreenObject.template_directory + '/inc/admin/welcome-screen/img/ajax-loader.gif" /></div>');
            },
            success    : function(data){
				jQuery("#temp_load").remove(); /* Remove loading gif */
                jQuery('#'+ data).parent().remove(); /* Remove required action box */

                var openlab_lite_actions_count = jQuery('.openlab-txtd-actions-count').text(); /* Decrease or remove the counter for required actions */
                if( typeof openlab_lite_actions_count !== 'undefined' ) {
                    if( openlab_lite_actions_count == '1' ) {
                        jQuery('.openlab-txtd-actions-count').remove();
                        jQuery('.openlab-txtd-tab-pane#actions_required').append('<p>' + openlabLiteWelcomeScreenObject.no_required_actions_text + '</p>');
                    }
                    else {
                        jQuery('.openlab-txtd-actions-count').text(parseInt(openlab_lite_actions_count) - 1);
                    }
                }
            },
	...
	function openlab_welcome_page_tabs(event) {
        ...
        jQuery(".openlab-txtd-tab-pane").not(tab).css("display", "none");
        jQuery(tab).fadeIn();
	}
    ...
    jQuery(".openlab-txtd-nav-tabs a").click(function(event) {
